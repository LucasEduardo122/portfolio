...
export default function StacksGrid({name, url, image, type} : StacksGridProps) {
    return (
        <a title={name} target="_blank" className="h-full" href={url}>
            <div className="rounded-xl border bg-card text-card-foreground shadow flex flex-col hover:bg-[#121212] hover:scale-105 transform transition-all duration-500 ease-in-out items-center justify-center h-full">
                <div className="flex flex-col space-y-1.5 p-6">
                    <Image alt={name} title={name} fetchPriority="high" width={80} height={80} decoding="async" data-nimg="1" className="rounded-lg max-w-80 max-h-80 overflow-hidden bg-cover bg-center hover:scale-110 transform transition-all duration-500 ease-in-out" src={image} style={{ color: "transparent" }} /></div>
